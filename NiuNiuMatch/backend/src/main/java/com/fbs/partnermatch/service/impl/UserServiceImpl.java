...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>
    ...
    public List<User> searchUsersByTags(List<String> tagNameList) {
        ...
        Gson gson = new Gson();
        // 2. 在内存中判断是否包含要求的标签
        return userList.stream().filter(user -> { //流式处理
            String tagsStr = user.getTags();
            //json字符串反序列化为java对象
            Set<String> tempTagNameSet = gson.fromJson(tagsStr, new TypeToken<Set<String>>() {
            }.getType());
            //ofNullable封装一个可能为空的对象，如果为空，则取orElse中的值（即赋一个默认值，避免出现空 ）
    ...
    public int updateUser(User user,User loginUser) {
        ...
        //如果是管理员，允许更新任意用户
        //如果不是管理员，之允许更新当前自己的信息
        if(!isAdmin(loginUser) && userId!=loginUser.getId()){//不是管理员且要更新的用户id不等于当前登录的用户id
    ...
    public User getLoginUser(HttpServletRequest request) {
        ...
        if(userObj==null){
            throw new BusinessException(ErrorCode.NO_AUTH);//抛出异常：无权限
        }
        return (User) userObj;
    }
